...
(function () {
  ...
  // ========== QUIZ ==========
  // Sample quiz data using known short surahs (verified text)
  const QUIZ_BANK = {
    112: {
      name: 'Al-Ikhlas',
      ayahs: [
    ...
    1: {
      ...
      ayahs: [
        ...
        { n: 7, ar: 'صِرَاطَ الَّذِينَ أَنْعَمْتَ عَلَيْهِمْ غَيْرِ الْمَغْضُوبِ عَلَيْهِمْ وَلَا الضَّالِّينَ', en: 'The path of those upon whom You have bestowed favor, not of those who have evoked [Your] anger or of those who are astray.' }
      ]
    }
  };
  ...
  function initQuizPage() {
    ...
    if (juzSel && !juzSel.children.length) {
      juzSel.innerHTML = '<option value="30">Juz 30 (sample)</option><option value="1">Juz 1 (sample)</option>';
    }
    const surahSel = document.getElementById('quizSurahSelect');
    if (surahSel && !surahSel.children.length) {
      surahSel.innerHTML = Object.entries(QUIZ_BANK).map(([n, s]) =>
  ...
  function startQuiz() {
    ...
    if (scope === 'surah') {
      ...
      bank = QUIZ_BANK[112]; // sample
    }
    if (!bank || bank.ayahs.length < 2) {
      alert('Not enough ayahs for a quiz in this selection.');
      return;
    }

    const questions = [];
    const ayahs = bank.ayahs;
    // Continue the Ayah type
    for (let i = 0; i < Math.min(5, ayahs.length - 1); i++) {
      const correct = ayahs[i + 1];
      const options = [correct];
      // distractors
      while (options.length < 4) {
        const rand = ayahs[Math.floor(Math.random() * ayahs.length)];
        if (!options.find(o => o.n === rand.n)) options.push(rand);
      }
      shuffle(options);
      questions.push({
        type: 'Continue the Ayah',
        prompt: ayahs[i].ar,
        options: options.map(o => ({ text: o.ar, correct: o.n === correct.n })),
        correctAyah: correct
      });
    }
  ...
  function showQuizQuestion() {
    ...
    opts.innerHTML = q.options.map((o, i) =>
      `<button class="quiz-opt" data-idx="${i}">${o.text}</button>`
    ).join('');
